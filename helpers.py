...
# Function, that draws Cartesian Coordinate system (from https://stackoverflow.com/questions/13430231/how-i-can-get-cartesian-coordinate-system-in-matplotlib)
...
# Implementing a function to validate user input:
...
def promptParam(value):
    try:
        if abs(int(value)) > 25:
            exit(
                "Sorry, but this calculator can handle parameters which have absolute value below 25 :(")
    except ValueError:
...
def quadFunc(aP, bP, cP):
    ...
    if yZero < 0:
        ...
        if pointedUp:
            ...
            if yZero == 0:
                xBorder = 2*xBorder + 2
                if xZero > 0:
                    grid(-2, xBorder, -yBorder, yBorderInter)
                if xZero < 0:
                    grid(-xBorder, 2, -yBorder, yBorderInter)
            else:
                grid(-xBorder, xBorder, -yBorder, yBorderInter)
        elif not pointedUp:
            yBorder = abs(yZero) + 2
            ...
            if yZero == 0:
                xBorder = 2*xBorder + 2
                if xZero > 0:
                    grid(-2, xBorder, -yBorderInter, yBorder)
                if xZero < 0:
                    grid(-xBorder, 2, -yBorderInter, yBorder)
            else:
                grid(-xBorder, xBorder, -yBorderInter, yBorder)
    ...
    # Define the function
    if yZero == 0:
        x = np.arange(-(abs(xBorder)+1), (abs(xBorder)+1), 0.1)
    else:
    ...
    if True:
        ...
        if a == -1:
            wrtA = "-"
    plt.title(f"Function: {wrtA}x²{wrtB}{wrtC}\n\n")

    # Print starting coordinate of the parrabola:
    print(
        f"Function: {wrtA}x²{wrtB}{wrtC}\n----------------------------------------------------------\nZEROES: \n")
